    ...
    def __call__(self, imgs):
        ...
        for i in range(0, self.index):
            ...
            if img.mode == 'I':
                ...
                # 为UntypedStorage指定dtype参数
                pic = torch.ByteTensor(torch.UntypedStorage.from_buffer(img.tobytes(), dtype=torch.uint8))
        ...
        for i in range(self.index, len(imgs)):
            ...
            if label.mode == 'I':
                ...
                label_tensor = torch.ByteTensor(torch.UntypedStorage.from_buffer(label.tobytes(), dtype=torch.uint8))  # 5080专用
            # PIL image mode: 1, L, P, I, F, RGB, YCbCr, RGBA, CMYK
            if label.mode == 'YCbCr':
                nchannel = 3
            elif label.mode == 'I;16':
...
class LabelEncoding(object):
    ...
    def __call__(self, imgs):
        ...
        if label.ndim == 3:  # 如果是3通道图像
            ...
            new_label[label > 0] = 1  # 非零像素为前景

        # # 处理单通道和二值标签
        # if label.ndim == 3:  # 如果是3通道图像
        #     # 二分类：所有前景（红色和绿色）都标记为类别1
        #     new_label = np.zeros((label.shape[0], label.shape[1]), dtype=np.uint8)
            
        #     # 识别所有前景区域（红色或绿色）
        #     red_foreground = (label[:, :, 0] > 200) & (label[:, :, 1] < 50) & (label[:, :, 2] < 50)
        #     green_foreground = (label[:, :, 1] > 200) & (label[:, :, 0] < 50) & (label[:, :, 2] < 50)
        #     new_label[red_foreground | green_foreground] = 1  # 前景类别
