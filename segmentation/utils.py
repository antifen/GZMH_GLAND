...
# ===================== 方案2：快速降采样豪斯多夫距离 =====================
def fast_hausdorff(maskA, maskB, sample_step=5):
    pA = np.argwhere(maskA)[::sample_step]
    pB = np.argwhere(maskB)[::sample_step]
    
    if len(pA) == 0 or len(pB) == 0:
        return 150.0
    
    d1 = directed_hausdorff(pA, pB)[0]
    d2 = directed_hausdorff(pB, pA)[0]
    return max(d1, d2)
...
def accuracy_pixel_level(output, target):
    ...
    results = np.zeros((6,), float)  #5080专用
...
def nuclei_accuracy_object_level(pred, gt):
    ...
    for i in range(1, Ng+1):
        ...
        haus += fast_hausdorff(pred_i, gt_i)
...
# 添加compute_hausdorff参数控制Hausdorff计算
def gland_accuracy_object_level(pred, gt, compute_hausdorff=True):
    ...
    Ns = len(np.unique(pred_labeled)) - 1
    gt_labeled = morph.label(gt, connectivity=2)
    gt_labeled = morph.remove_small_objects(gt_labeled > 0, 3)   # remove 1 or 2 pixel noise in the image
    gt_labeled = morph.label(gt_labeled, connectivity=2)
    Ng = len(np.unique(gt_labeled)) - 1
    ...
    for i in range(1, Ng + 1):
        ...
        if obj_no.size == 0:   # no intersection object
            dice_i = 0
            iou_i = 0
            haus_i = 150 if compute_hausdorff else 0
        else:
            # find max overlap object
            obj_areas = [np.sum(overlap_parts == k) for k in obj_no]
            ...
            if compute_hausdorff:
                haus_i = fast_hausdorff(pred_i, gt_i)
            else:
                haus_i = 0
    ...
    for j in range(1, Ns + 1):
        ...
        if obj_no.size == 0:
            dice_j = 0
            iou_j = 0
            haus_j = 150 if compute_hausdorff else 0
        else:
            # find max overlap gt
            ...
            if compute_hausdorff:
                haus_j = fast_hausdorff(pred_j, gt_j)
            else:
                haus_j = 0
...
# 添加compute_hausdorff参数控制Hausdorff计算
def gland_accuracy_object_level_all_images(pred, gt, compute_hausdorff=True):
    """ Compute the object-level metrics between predicted and groundtruth """
...
def split_forward_unet(model, input, size, overlap, outchannel=3):
    '''
...
def split_forward(model, input, size, overlap, outchannel=3):   #适用于deeplabv3+
    '''
    split the input image for forward process (modified for DeepLabv3+)
    '''
    b, c, h0, w0 = input.size()
    device = input.device  # 获取输入数据的设备（应与模型一致）

    # zero pad for border patches
    pad_h = 0
    if h0 - size > 0:
        pad_h = (size - overlap) - (h0 - size) % (size - overlap)
        tmp = torch.zeros((b, c, pad_h, w0), device=device)  # 在同一设备上创建临时张量
        input = torch.cat((input, tmp), dim=2)

    if w0 - size > 0:
        pad_w = (size - overlap) - (w0 - size) % (size - overlap)
        tmp = torch.zeros((b, c, h0 + pad_h, pad_w), device=device)
        input = torch.cat((input, tmp), dim=3)

    _, c, h, w = input.size()

    # 在设备上初始化output和count
    output = torch.zeros((b, outchannel, h, w), device=device)
    count = torch.zeros((h, w), device=device) + 1e-8  # 避免除零
    ...
    for i in range(0, h-overlap, size-overlap):
        ...
        for j in range(0, w-overlap, size-overlap):
            ...
            input_patch = input[:,:,i:r_end,j:c_end]
            with torch.no_grad():
                output_patch = model(input_patch)  # 模型输出应在同一设备上

            ind2_s = j+overlap//2 if j>0 else 0
            ind2_e = j+size-overlap//2 if j+size<w else w
            
            # 确保output_patch与output设备一致（通常由model保证）
            output[:,:,ind1_s:ind1_e, ind2_s:ind2_e] += output_patch[:,:,ind1_s-i:ind1_e-i, ind2_s-j:ind2_e-j]
            count[ind1_s:ind1_e, ind2_s:ind2_e] += 1

    # 归一化
    output = output / count.unsqueeze(0).unsqueeze(0)
    output = output[:,:,:h0,:w0]  # 裁剪回原始尺寸
